  ...
  return new Promise((resolve) => {
    const interval = setInterval(() => {
      if (currentIndex < fullText.length) {
        ...
        element.innerHTML = htmlContent.replace(
          /<strong>"([^"]+)"<\/strong>/,
          (match, p1) => {
            ...
            if (currentIndex <= beforeStrong) {
              ...
              const partialStrong = p1.substring(
                0,
                currentIndex - beforeStrong,
              );
        ...
        if (parts) {
          ...
          if (currentIndex <= before.length) {
            ...
            const strongPart = displayText.substring(
              before.length,
              currentIndex,
            );
            element.innerHTML =
              before + '<strong>"' + strongPart + '"</strong>';
          } else {
            const afterPart = displayText.substring(
              before.length + strongText.length,
            );
            element.innerHTML =
              before + '<strong>"' + strongText + '"</strong>' + afterPart;
          }
        } else {
...
function createMediumConfidenceResponse(topResult) {
  ...
  const cleanQuestion = questionText.endsWith("?")
    ? questionText.slice(0, -1)
    : questionText;
...
async function ask(question) {
  ...
  const loadingBubble = appendBubble(
    '<span class="muted"><i class="bi bi-search"></i> </span>',
    "bot",
  );
  ...
  try {
    ...
    const errorBubble = appendBubble(
      '<span class="muted"><i class="bi bi-x-circle"></i> </span>',
      "bot",
    );
