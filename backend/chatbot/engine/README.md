...
> **Stack** : MiniLM · FAISS · Google Gemini (1.5 Flash) · TF-IDF · Django
...
Question de l'étudiant
   ...
   faiss_search.py     →  Top-3 résultats (score cosinus)
         │
         ├── score ≥ 0.55 ──→  prompt_builder.py  →  llm_client.py  →  Réponse Gemini
         │
...
├── llm_client.py         # Client API Google Gemini (1.5 Flash)
├── prompt_builder.py     # Construction du prompt + contexte FAISS
...
pip install google-generativeai     # Client LLM Gemini
```

Le niveau LLM utilise l'**API Google Gemini**. Aucun serveur local n'est requis ;
il suffit de fournir une clé API via la variable d'environnement `GEMINI_API_KEY`
(voir `.env.example`). Obtenez une clé sur https://aistudio.google.com/app/apikey.

| Variable | Défaut | Description |
|----------|--------|-------------|
| `GEMINI_API_KEY` | — | Clé API Google Gemini (obligatoire pour le niveau LLM) |
...
Client de l'**API Google Gemini** (`google-generativeai`). Fournit la même
interface publique que l'ancienne version Ollama, de sorte que `rag_pipeline.py`,
`views.py` et `prompt_builder.py` restent inchangés.
...
> Si `GEMINI_API_KEY` n'est pas définie, le niveau LLM est désactivé : le
> pipeline bascule proprement vers TF-IDF / OFFBASE.

### Fonctions

#### `generate(prompt, level="llm", model) → str`
...
reponse = generate(prompt, level="llm")
print(reponse)   # Texte complet de la réponse
```

| Paramètre | Type | Défaut | Description |
|-----------|------|--------|-------------|
| `prompt` | `str` | — | Prompt construit par `build_prompt()` |
| `level` | `str` | `"llm"` | `"llm"` ou `"direct"` (paramètres de génération) |
| `model` | `str` | `gemini-1.5-flash` | Modèle Gemini |
| **Retour** | `str` | — | Texte de la réponse |

---

#### `generate_stream(prompt, level="llm", model) → Iterator[str]`

Génération en **streaming** : yield chaque chunk dès sa production. Utilisé pour les réponses progressives dans l'interface web (SSE).
...
(via `list_models()`, sans consommer de tokens).

```python
status = check_availability()
# {"available": True, "model": "gemini-1.5-flash", "error": None}
```

---

### Paramètres de génération (`GenerationConfig`)

| Niveau | `temperature` | `max_output_tokens` | `top_p` |
|--------|---------------|---------------------|---------|
| `llm` | `0.3` | `512` | `0.90` |
| `direct` | `0.1` | `150` | `0.85` |

Le message système (langue française, ancrage strict sur le contexte,
réponses courtes) est transmis via `system_instruction` de Gemini.

### Résilience

Un **circuit-breaker** léger ouvre le circuit après 3 échecs consécutifs
(réinitialisation après 30 s), évitant de marteler l'API en cas de panne.
...
Construit le prompt complet envoyé au LLM. Intègre l'historique de conversation et les contextes FAISS pour que le LLM génère une réponse précise et ancrée dans les données de SUP'PTIC.

> **Note Gemini** : le message système est transmis séparément via
> `system_instruction` (voir `llm_client.py`). Les anciens marqueurs de template
> `<|system|>…<|assistant|>` présentés ci-dessous sont conservés à titre
> illustratif de la structure logique du contexte.
...
# Tester le client Gemini (nécessite GEMINI_API_KEY)
